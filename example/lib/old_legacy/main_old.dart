...
// import 'package:firebase_core/firebase_core.dart'; // Removed for legacy compatibility
...
void main() async {
  ...
  // Initialize Firebase early for iOS (commented out for legacy compatibility)
  try {
    // await Firebase.initializeApp();
    print('🔥 Firebase initialization skipped in legacy version');
  } catch (e) {
...
Future<void> _requestBatteryOptimizationBypass() async {
  try {
    ...
    final isIgnoring = true; // Legacy fallback
    ...
    if (isIgnoring) {
      ...
      final requested = true; // Legacy fallback
      if (requested) {
...
class MyAppState extends ConsumerState<MyApp> with WidgetsBindingObserver {
  ...
  void _handleAppResume() async {
    try {
      ...
      // final // sipUserCubit // Legacy removed = ref.read(// sipUserCubit // Legacy removedProvider); // Legacy provider removed
      final helper = ref.read(sipHelperProvider);
      ...
      print('📊 SIP Status:');
      print('📊 Has saved user: ${false /* sipUserCubit.state != null - Legacy removed */}');
      print('📊 Is registered: ${false /* sipUserCubit.isRegistered - Legacy removed */}');
      print('📊 Helper registered: ${helper.registered}');
      
      // Take back SIP control from background service only if no active calls
      if (false /* sipUserCubit.state != null - Legacy removed */) {
        ...
        if (!helper.registered) {
          ...
          // await sipUserCubit.forceReconnect(); // Legacy removed
  ...
  void _transferToBackgroundService() async {
    try {
      ...
      // Legacy provider removed - commenting out for compilation
      ...
      // Legacy code commented out for compilation
      if (false /* helper.registered && sipUserCubit.state != null */) {
        ...
        // helper.addSipUaHelperListener(_BackgroundCallListener()); // Legacy removed
  ...
  Widget build(BuildContext context) {
    // final theme = ref.watch(themeNotifierProvider); // Legacy provider removed
    
    return MaterialApp(
      title: 'SIP Phone',
      debugShowCheckedModeBanner: false,
      theme: ThemeData.light(), // Use default theme since provider is removed
      navigatorKey: navigatorKey,
      initialRoute: '/',
      onGenerateRoute: (settings) => MaterialPageRoute(builder: (context) => Container()), // Stub route
    );
  }
}
