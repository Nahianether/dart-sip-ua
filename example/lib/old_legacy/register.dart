...
class _MyRegisterWidget extends ConsumerState<RegisterWidget>
  ...
  void registrationStateChanged(RegistrationState state) {
    ...
    if (mounted) {
      ref.read(registrationStateProvider.notifier).state = state.state ?? RegistrationStateEnum.UNREGISTERED;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final providerState = ref.watch(registrationStateProvider);
    final helperState = helper!.registerState.state;
    final registerStateEnum = providerState ?? helperState ?? RegistrationStateEnum.UNREGISTERED;
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (registerStateEnum == RegistrationStateEnum.REGISTERED) ...[
                SizedBox(
              ...
              ] else if (registerStateEnum == RegistrationStateEnum.REGISTRATION_FAILED) ...[
                Row(
                  children: [
                    Expanded(
                      child: ElevatedButton(
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        Icon(
                          registerStateEnum == RegistrationStateEnum.REGISTERED
                              ? Icons.check_circle
                              : registerStateEnum == RegistrationStateEnum.REGISTRATION_FAILED
                                  ? Icons.error
                                  : Icons.pending,
                          color: registerStateEnum == RegistrationStateEnum.REGISTERED
                              ? Colors.green
                              : registerStateEnum == RegistrationStateEnum.REGISTRATION_FAILED
                                  ? Colors.red
                                  : Colors.orange,
                    ...
                    if (registerStateEnum == RegistrationStateEnum.REGISTRATION_FAILED) ...[
                      const SizedBox(height: 8),
                      Text(
